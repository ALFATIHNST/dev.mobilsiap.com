...
export default function SaleForm() {
  ...
  const uploadPhoto = async (supabase: ReturnType<typeof createClient>, file: File, submissionId: string, position: string): Promise<string | null> => {
    ...
    const { error } = await supabase.storage.from('car-sale-photos').upload(path, file, { upsert: true });
    if (error) return null;
    const { data: { publicUrl } } = supabase.storage.from('car-sale-photos').getPublicUrl(path);
    return publicUrl;
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('car_sale_requests').insert({
        id: submissionId,
      ...
      if (error) {
        console.error('Car sale submission error:', (error as any).message);
  ...
  if (submitted) {
    return (
      <section id="sale-form" className="py-20 px-4 sm:px-6 lg:px-8" style={{ background: 'rgba(8,8,14,0.95)' }}>
    ...
    <section id="sale-form" className="py-20 px-4 sm:px-6 lg:px-8" style={{ background: 'rgba(8,8,14,0.95)' }}>
      <div className="max-w-3xl mx-auto">
        <div className="text-center space-y-3 mb-10">
          <span className="section-label">Formulir Jual Mobil</span>
          <h2 className="text-section-heading font-extrabold text-white" style={{ fontWeight: 800 }}>
            Ajukan Penjualan Mobil Anda
          </h2>
          <p className="text-white/50 text-lg">
            Isi formulir di bawah ini dengan lengkap untuk mengajukan penjualan mobil.
          </p>
        </div>
